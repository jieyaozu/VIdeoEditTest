...
//                muxAudioTrack = mMediaMuxer.addTrack(format);
    ...
    private void initVideoCodec() {
        ...
        info.outputWidth = 720;
        info.outputHeight = 1280;
        info.rotation = videoRotation;

        MediaFormat mediaFormat;
        if (info.rotation == 0 || info.rotation == 180) {
            mediaFormat = MediaFormat.createVideoFormat("video/avc", info.outputWidth, info.outputHeight);
        } else {
            mediaFormat = MediaFormat.createVideoFormat("video/avc", info.outputHeight, info.outputWidth);
        }
        //设置视频的编码参数
        ...
        outputSurface = new OutputSurface(context, info);
        outputSurface.setBlurLevel(mCurrentBlurLevel);

        videoDecoder.configure(videoFormat, outputSurface.getSurface(), null, 0);
        videoDecoder.start();//解码器启动
    }

    private BlurLevel mCurrentBlurLevel;

    public void setBlurLevel(BlurLevel level) {
        mCurrentBlurLevel = level;
    }
